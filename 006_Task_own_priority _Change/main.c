...
// Same task priority going to share processor
// 

int main(){
	ledInit();
	// Create tasks
	xTaskCreate(vRedLedToggleTask, "Red Led Blink", 100, NULL, 2,&red_handle);
	xTaskCreate(vGreenLedToggleTask, "Green Led Blink", 100, NULL, 1 ,&green_handle);
	xTaskCreate(vBlueLedToggleTask, "Blue Led Blink", 100, NULL , 2,&blue_handle);
	// Start Schedular
	vTaskStartScheduler();
